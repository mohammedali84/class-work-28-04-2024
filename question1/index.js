...
function calculateArea(radius2 = 10) {
    let dis = 2 * 3.14 * (radius2 * radius2);
    console.log(dis);
}
calculateArea();
//
...
let user = 15;
if (user < 18) {
    console.log("User is Minor and age is", user);
}
else {
    console.log("Adult");
}
...
const positive_number = (number_array) => {
    let count = 0;
...
const wordsStartingWithAArray = wordsStartingWithA(wordArray);
...
const stringsArray = ["apple", "banana", "orange", "kiwi", "pineapple"];
const filteredArray = filterByLength(stringsArray, 5);
...
const arrayWithDuplicates = [1, 2, 3, 4, 1, 5, 2, 6, 3];
...
const incrementedArray = incrementAll(integersArray);
...
const numbersArray1 = [1, 2, 3, 4, 2, 2, 3, 5];
const countOfTwos = countOccurrences(numbersArray1, 2);
...
const namesArray = ["John", "Jane", "Doe", "Alice"];
...
const occurrencesCount = countCharacterOccurrences(inputString, characterToCount);
